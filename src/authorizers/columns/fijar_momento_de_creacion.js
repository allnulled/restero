module.exports = function (deployer) {
    return async function (request, response, parametro, columna_id) {
        try {
            deployer.utilities.tracear("deployer.authorizers.columns.fijar_momento_de_creacion");
            ...
            request.hql_data.item[columna_id] = `${
                ...
                deployer.utilities.obtener_string_padeado(fecha.getHours(), 2)
            }:${
                deployer.utilities.obtener_string_padeado(fecha.getMinutes(), 2)
            }:${
                deployer.utilities.obtener_string_padeado(fecha.getSeconds(), 2)
            }`;
        } catch (error) {
            console.error("Error en «src/authorizers/columns/fijar_momento_de_creacion.js»");
            console.error(error);
            throw error;
        }
    };
