    ...
    return async function (request, response, parametro) {
        try {
            ...
            if (typeof autentificacion !== "object") {
                throw new Error("Autentificación requerida por autorizador «si_eliminable_por_mismo_usuario»");
            }
            ...
            if(!es_mismo_usuario) {
                throw new Error("Operación no permitida por «si_eliminable_por_mismo_usuario»");
            }
        } catch (error) {
            console.error("Error en «src/authorizers/si_eliminable_por_mismo_usuario.js»");
            console.error(error);
            throw error;
        }
    };
