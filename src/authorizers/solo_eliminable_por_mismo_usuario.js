module.exports = function (deployer) {
    return async function (request, response, parametro) {
        try {
            deployer.utilities.tracear("deployer.authorizers.solo_eliminable_por_mismo_usuario");
            ...
            if (typeof autentificacion !== "object") {
                throw new Error("Autentificación requerida por autorizador «solo_eliminable_por_mismo_usuario»");
            }
            ...
            if(objetos_coincidentes.length === 0) {
                throw new Error("Objeto de tabla no reconocido según identificador en «solo_eliminable_por_mismo_usuario»: «" + id + "»");
            }
            ...
            if(!es_mismo_usuario) {
                throw new Error("Operación no permitida por «solo_eliminable_por_mismo_usuario»");
            }
        } catch (error) {
            console.error("Error en «src/authorizers/solo_eliminable_por_mismo_usuario.js»");
            console.error(error);
            throw error;
        }
    };
